...
UNICAST_HOSTS='["'"$NAMESPACE_PREFIX"'-master-vm0:9300","'"$NAMESPACE_PREFIX"'-master-vm1:9300","'"$NAMESPACE_PREFIX"'-master-vm2:9300"]'
...
if [ ${CLUSTER_USES_DEDICATED_MASTERS} -ne 0 ]; then
    ...
    for i in $(seq 0 $((DATANODE_COUNT-1))); do
        UNICAST_HOSTS="$UNICAST_HOSTS\"${NAMESPACE_PREFIX}-data-vm$i:9300\","
    done
